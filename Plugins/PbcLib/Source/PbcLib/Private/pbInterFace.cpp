...
static int
_env_regfile(lua_State *L) {
	struct pbc_env * env = (struct pbc_env *)checkuserdata(L,1);
	const char * fileName = luaL_checkstring(L, 2);
	IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
	FString path = FPaths::ProjectContentDir();
	path += "LuaSource/Network/Pb/";
	FString ffn = ANSI_TO_TCHAR(fileName);
	path += ffn;
	IFileHandle* FileHandle = PlatformFile.OpenRead(*path);
	if (FileHandle) {
		uint32 len = (uint32)FileHandle->Size();
		uint8* buffer = new uint8[len];
		FileHandle->Read(buffer, len);
		delete FileHandle;
...
void lua_OpenLib(lua_State *L, lua_CFunction fn, const char *k)
{
