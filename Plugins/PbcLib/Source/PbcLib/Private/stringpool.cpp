...
struct _stringpool * 
_pbcS_new(void) {
	struct _stringpool * ret = (struct _stringpool *)malloc(sizeof(struct _stringpool) + PBC_PAGE_SIZE);
...
_pbcS_build(struct _stringpool *pool, const char * str , int sz) {
	...
	struct _stringpool *next = (struct _stringpool *)malloc(sizeof(struct _stringpool) + PBC_PAGE_SIZE);
