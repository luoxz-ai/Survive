...
#if 1
	// Protobuff3 中有做一个repated的优化，会把所有repeated numeric的消息默认加上[packed = true]
	// 这会导致pbc在解析这类消息时出现问题，下面的修改就是修复这个bug
	int raw_label = pbc_rmessage_integer(field, "label", 0, 0) - 1; // LABEL_OPTIONAL = 0
	f->label = raw_label;//pbc_rmessage_integer(field, "label", 0, 0) - 1; // LABEL_OPTIONAL = 0

	// 如果type时numeric的，将label改为LABEL_PACKED
	switch(f->type)
	{
	case PTYPE_DOUBLE:
    case PTYPE_FLOAT:
