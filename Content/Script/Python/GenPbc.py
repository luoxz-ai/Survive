...
def AutoGenPbc_Map(cmdList , outFile):
    ...
    for cmd in cmdList:
        ...
        and not re.search(msgExclude[1], cmd):
            ...
            if res:
                cmdStr = res.group()
                outputList.append("    {0} = '{1}',".format(cmdStr[4:] ,cmdStr.replace("CMD_" , "Cmd.")) )
...
def AutoGenPbc_Id(cmdList , outFile):
    ...
    for cmd in cmdList:
        ...
        and not re.search(msgExclude[1], cmd):
            ...
            if res:
                outputList.append("    {0} = '{1}',".format(res[0][4:] , res[1]) )
    outputList.append("}")
...
def AutoGenPbc(rootDir):
    inFile = rootDir + "/Proto/xCmd.proto"
