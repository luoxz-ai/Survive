...
class SURVIVE_API MessageManager
{
public:

	using LuaRcvPbcEvt = std::function<void(const std::string)>;
	LuaRcvPbcEvt OnLuaRcvPbcMsg;

    explicit MessageManager(const std::string &remote = "mmm.io:9876");
	~MessageManager();

	void PostConnect();
	void PostDisconnect();

	void SendPbcMessage(const std::string& buffer);
